...
A fifth is published but not built by `build.py`: `Bergmann-CV-Artist` comes
from a committed spec in `custom/`, generated by CI and linked from the README.
See [Artist CVs](#artist-cvs).
...
- **Write output to `custom/`, never `output/`** — with one deliberate exception.
  ...
  were canonical. The exception is `Bergmann-CV-Artist`: CI itself generates it
  into `output/` under a fixed name, because the README links to it and a
  published link has to stay current. Publishing another means editing the
  workflow and the README, not dropping a file into `output/` by hand.
...
pre-2019 design record promoted rather than archived. Those credits — projection,
installation, direction, lighting, film — are the bulk of the professional
practice and appear in no canonical document but FullRecord, which is right for
an academic CV and exactly wrong for an arts jury.

**`Bergmann-CV-Artist` is a published document.** CI builds it from
`custom/artist-cv-canada-council.yaml` and the README links to it, so it is not
an ad hoc artefact — treat a change to that spec the way you would treat a
change to `build.py`. It runs about seven pages: the selected highlights, then
recent creative work, residencies, awards, grants, the full design record broken
out by kind, affiliations, press, and education.

`custom/artist-cv.yaml` is the same document cut to three pages, kept as a
starting point for applications with a page limit. It is **not** published;
anything size-specific gets built on request.

### The three things that make an artist CV work

**Curate by `selected: true`, not by `limit:`.** Every other filter in the repo
sorts by date, and an arts jury does not want the newest twelve things — it wants
the work that defines the practice. On this record a date sort with `limit: 12`
drops *He Left Quietly* (Best Production, SummerWorks) and *Accidental Death of
an Anarchist* (Yale Rep / Berkeley Rep), which is close to the opposite of the
intent. Sixteen entries carry the flag, chosen on juried awards, international
presentation, gallery exhibition, and premieres.

`selected:` is the one flag in `cv.yaml` that records taste rather than fact.
Michael revises it; don't quietly re-curate it while doing something else, and do
raise it if a new work clearly belongs.

**`dates: year`.** The left column carries the year alone. A specific run of
dates already sits in the citation, and repeating it beside every line says the
same thing twice. Month precision remains the default everywhere else.

**`contact_email: email_personal`.** Artist CVs carry `michael@bergmann.ai`
rather than the TMU address, because juries and funders write back to the
personal one. The address lives in `cv.yaml`'s `personal` block; the spec names
the key rather than repeating the string.

### Judgement calls already encoded

Worth knowing before copying either spec, because each looks like an oversight
until you know it was a decision:

- **Conference-travel grants are excluded** from the funder CV. They funded
  attendance rather than practice, and read as academic noise beside a
  production budget. The Shipwreck travel grant survives, having funded the work
- **Memberships are filtered by exclusion, not by match**, so a membership added
  to `cv.yaml` later appears by default instead of silently dropping out. Only
  the Canadian Artificial Intelligence Association and the Diagonal reading group
  are cut; ADC/IATSE, USITT, and CITT all stay
- **Setsubun sits under Residencies** even though it was a mentorship role. The
  entry names him as mentor, which is the distinction a reader needs

### Building one

```bash
S=.claude/skills/bergmann-cv/scripts/custom_cv.py
python3 $S custom/artist-cv-canada-council.yaml --out output/Bergmann-CV-Artist.md
./convert.sh Bergmann-CV-Artist
```

For a one-off — a residency with a two-page cap, a gallery that wants exhibitions
only — copy a spec into `custom/<application>.yaml`, adjust, and build it to
`custom/`, not `output/`. The copy is worth committing: it records how that
application was framed, and the next one starts from it. Only `custom/*.yaml` is
tracked; generated files there are ignored.
