...
cv.yaml  ──►  build.py  ──►  output/*.md  ──►  convert.sh  ──►  output/*.docx + *.pdf
   ...
   └──►  custom_cv.py + custom/artist-cv-*.yaml ┘ (Bergmann-CV-Artist, in CI)
```
...
1. **Edit `cv.yaml` only** for CV content. Never edit files in `output/` — they are overwritten on every build. The artist CV there (`Bergmann-CV-Artist`) is generated in CI from `custom/artist-cv-canada-council.yaml`; change the spec, not the output.
...
4b. **Mentorship and HQP**: tag any entry whose substance is mentorship or training with `mentorship: true` rather than copying it into a section of its own; list supervised people named in a citation under `hqp: [...]`. Neither renders in the four documents — they exist for the tri-agency CV, and for being able to answer "who did I supervise on that" later.
4c. **`selected: true`** marks a career-highlight work and drives the artist CV's "Selected Works" list (`custom/artist-cv.yaml`, `custom/artist-cv-canada-council.yaml`). It is the one flag that records taste rather than fact; revise it as the practice changes.
...
- **No one in the record counts as HQP** (August 2026). The `hqp:` key exists for the tri-agency CV, but no co-author or collaborator currently in `cv.yaml` was a person Michael supervised. Don't tag the *Augmented Symphony* co-authors, the *Almost Alchemy* group, or Carravetta/Endahl/Robinson — asked and answered
- The Oulu closing keynote ("Wellness for who?") is a **talk, not mentorship**. Michael did mentor students at Oulu, but that was the separate AVARA 2024 project, which the talk discussed
- Setsubun stays under **Residencies** on the artist CV; the entry names him as mentor, which is the distinction a reader needs
- Artist CVs carry `michael@bergmann.ai`, not the TMU address — juries and funders write back to the personal one
